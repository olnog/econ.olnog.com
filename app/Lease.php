...
class Lease extends Model
{
  ...
  public static function bad($contractID, $reason){
    ...
    \App\History::new($contract->userID, 'lease',
      "All the leases you had for your " . $contract->landType
      . "  are no longer valid because you " . $reason);
    foreach ($leases as $lease){
      \App\History::new($lease->userID, 'lease', "Your lease with "
        . $contractor->name . "  for " . $contract->landType
        . " is no longer valid because they " . $reason);
      $lease->active = false;
      $lease->save();
    }
  }
  ...
  public static function use($landType, $userID){
    ...
    if (!\App\Land::doTheyOwn($lease->landType, $contractor->id)){
      ...
      \App\History::new($contractor->id, 'lease', $leasor->name
        . " didn't have enough money, so their lease was canceled." );
      \App\History::new($leasor->id, 'lease',
        "You didn't have enough money, so your lease was canceled with "
        . $contractor->name . " for " . $contract->landType);
      $lease->active = false;
      $lease->save();
      return false;
    ...
    $leasor->save();
    return "Clacks: <span class='fn'>-" . $contract-> price . "</span> ("
      . ucfirst($lease->landType) . " Lease) ";
  }
}
